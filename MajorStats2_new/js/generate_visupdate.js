...
function generateVis() {
    ...
    var bubbles = d3.select(".bubbles")
        .selectAll("circle")
        .data(filtered_dataset, function(d) { return d.Team1; });
    ...
    bubbles
        ...
        .duration(interval)
        .ease(d3.easeQuad)
        .attr("cx", function(d) { return xScale(+d.Offensive_Yards_Allowed); })
        .attr("cy", function(d) { return yScale(+d.Total_Wins); })
        .attr("r", function(d, i) {
            return rScale(+d.Total_Wins);
        })
        .style("opacity", function(d) {
            if (checked_country.length > 0 && !checked_country.includes(d.Team1)) {
    ...
    texts.transition()
        .duration(interval)
        .attr("x", function(d) { return xScale(+d.Offensive_Yards_Allowed) + rScale(+d.Total_Wins); })
        .attr("y", function(d) { return yScale(+d.Total_Wins); })
    ...
    bubbles.enter()
        .append("circle")
        .attr("id", function(d) { return "circle_" + d.Team1; })
        .attr("cx", function(d) { return xScale(+d.Offensive_Yards_Allowed); })
        .attr("cy", function(d) { return yScale(+d.Total_Wins); })
        ...
        //.style("fill", function(d) {return colors_areas[d.Region];})
    ...
    texts.enter()
        .append("text")
        .attr("id", function(d) { return "text_" + d.Team1 })
